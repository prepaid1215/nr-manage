...
마지막 업데이트: 2026-08-29 (마감 계산기 라인 합계 규칙 반영)
...
- 마감 계산기 라인 합계 목표 자동 배분, 인증직급별 소실적 지급 기준선,
  부족분 분산 배치, 소실적 지급 구간 안내와 `[이대로 둔다]`
...
1. **마감 계산기 새 규칙 사업자 확인** — 라인 합계 + 소실적 기준선 규칙은 구현·테스트까지
   끝났다(`v2/docs/closing-calculator.md`). 사무실에서 확인받을 것:
   - 분산 배치 기준 금액 100,000원 (라인당 최대 2곳, 전체 4곳)
   - 인증직급을 모르는 회원은 기준선 0으로 두는 것
   - 마감 체크한 사업자 중 소실적이 기준선에 못 미치는 코드에 추가 매출이 들어가는 것
