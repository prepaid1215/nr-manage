...
create table if not exists public.nrc_sync_schedules (
  ...
  label text not null, source_account_id text not null, run_time time not null,
  timezone text not null default 'Asia/Seoul', enabled boolean not null default true,
  last_enqueued_on date, last_job_id uuid references public.nrc_sync_jobs(id) on delete set null,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create index if not exists nrc_sync_schedules_owner_time on public.nrc_sync_schedules(owner_id,run_time) where enabled;
...
declare v_owner_id uuid; v_source_account_id text; v_schedule record; v_job_id uuid; v_count integer:=0;
begin
  select owner_id,source_account_id into v_owner_id,v_source_account_id from public.nrc_sync_devices where id=p_device_id and owner_id=auth.uid();
  if v_owner_id is null then return 0; end if;
  for v_schedule in select * from public.nrc_sync_schedules
    ...
    order by run_time for update skip locked
  loop
    insert into public.nrc_sync_jobs(owner_id,source_account_id,status,message)
    values(v_owner_id,v_schedule.source_account_id,'QUEUED',v_schedule.label || ' 자동수집 대기 중...') returning id into v_job_id;
    update public.nrc_sync_schedules set last_enqueued_on=(now() at time zone timezone)::date,last_job_id=v_job_id,updated_at=now() where id=v_schedule.id;
    v_count:=v_count+1;
  end loop;
  return v_count;
end; $$;
