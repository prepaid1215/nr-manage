...
export async function boxGenealogyPage(root) {
  ...
  const nodeHtml = (row, level, path = new Set()) => {
    ...
    return `<li><button class="box-node ${rankTone(row)}${id === selectedId ? " selected" : ""}" data-member="${safe(id)}" type="button"><b>${safe(row.userName || "이름 없음")}</b><small>*${safe(id)}</small><small>${safe(row.rankName || "회원")}/${safe(row.rankMaxName || "회원")}</small><small>${safe(row.regDate || "-")}</small><em>본인 ${number(row.ordPv)} NV</em><span class="box-line-total">라인 전체 ${number(lineTotal(row))}</span></button>${hidden ? `<div class="box-more">아래 ${hidden}명 더 있음</div>` : ""}${showKids ? `<ul>${kids.map((kid) => nodeHtml(kid, level + 1, nextPath)).join("")}</ul>` : ""}</li>`;
  };
    ...
    if (query) {
      ...
      $("boxStage").innerHTML = matches.length
        ? `<div class="box-search-hits">${matches
            ...
            .map(
              (item) =>
                `<button class="box-node ${rankTone(item)}" data-member="${safe(item.userId)}" type="button"><b>${safe(item.userName || "이름 없음")}</b><small>*${safe(item.userId)}</small><small>${safe(item.rankName || "회원")}/${safe(item.rankMaxName || "회원")}</small><em>본인 ${number(item.ordPv)} NV</em><span class="box-line-total">라인 전체 ${number(lineTotal(item))}</span></button>`,
            )
            .join("")}</div>`
