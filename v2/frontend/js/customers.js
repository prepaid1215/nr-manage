...
const vcardPhoneLines=row=>{const seen=new Set();return[row.phone,row.contact_phone].filter((value)=>{const digits=String(value||'').replace(/\D/g,'');if(!digits||seen.has(digits))return false;seen.add(digits);return true}).map((value)=>`TEL;TYPE=CELL:${phone(value)}`).join('\r\n')};
