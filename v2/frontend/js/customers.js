...
export async function customersPage(root,me,options){
  ...
  const selectRepurchase=row=>{repurchaseTarget=row;$('repurchaseSelected').textContent=`${row.name} (개통 ${phone(row.phone)||'-'} · 연락 ${phone(row.contact_phone)||'-'}) 고객`;$('repurchaseAmountForm').hidden=false;$('repurchaseValue').value='';$('repurchaseDate').value=localDate();$('repurchaseValue').focus()};
