...
const vcardPhoneLines=row=>{const seen=new Set();return[[row.phone,'WORK'],[row.contact_phone,'CELL']].filter(([value])=>{const digits=String(value||'').replace(/\D/g,'');if(!digits||seen.has(digits))return false;seen.add(digits);return true}).map(([value,type])=>`TEL;TYPE=${type}:${phone(value)}`).join('\r\n')};
const vcardNote=row=>{const seen=new Set(),phones=[[row.phone,'개통번호'],[row.contact_phone,'연락번호']].filter(([value])=>{const digits=String(value||'').replace(/\D/g,'');if(!digits||seen.has(digits))return false;seen.add(digits);return true}).map(([value,label])=>`${label} ${phone(value)}`);return[...phones,row.network,row.plan,row.memo].filter(Boolean).join(' / ').replace(/\n/g,' ')};
...
export async function customersPage(root,me,options){
  ...
  $('customerActions').addEventListener('toggle',e=>{try{localStorage.setItem('customerActionsOpen',e.target.open?'1':'0')}catch{}});
  if(viewOwner){
    $('customerActions').open=true;
    ...
    $('viewOwnerBack').onclick=e=>{e.stopPropagation();document.querySelector('[data-page="organization"]')?.click()};
  }
  ...
  async function recordTransfer(field,amount,customer,date=localDate(),npay=false){
    if(!amount)return null;
    await bumpDailyActivity(field,amount,date);
    if(npay){
      const nextNpay=Number(me.nrc_pay_balance||0)+amount;
      const{error:npayError}=await supabase.from('profiles').update({nrc_pay_balance:nextNpay}).eq('id',me.id);
      if(!npayError)me.nrc_pay_balance=nextNpay;
    }
  ...
  const renderAmountCustomerMatches=query=>{
    ...
    const matches=rows.filter(r=>r.owner_id===me.id&&[r.name,r.phone,r.contact_phone,r.member_no].some(v=>String(v||'').toLowerCase().includes(q))).slice(0,8);
    box.innerHTML=matches.length?matches.map(r=>`<button type="button" class="repurchase-match" data-pick-customer="${esc(r.id)}">${esc(r.name)} <small>개통 ${esc(phone(r.phone)||'-')} · 연락 ${esc(phone(r.contact_phone)||'-')}</small></button>`).join(''):'<p class="help">일치하는 고객이 없습니다.</p>';
  ...
  const askAmount=(title,hint,opts={})=>new Promise(resolve=>{
    ...
    $('amountForm').onsubmit=e=>{
      ...
      done({amount:Number($('amountValue').value||0),customer:amountCustomer,date:$('amountDate').value||localDate(),npay:$('amountNpay').checked});
    };
  ...
  if(!viewOwner)$('exportCsv').onclick=()=>{const fields=[['name','고객명'],['phone','개통번호'],['contact_phone','연락가능번호'],['member_no','회원번호'],['network','통신사'],['plan','요금제'],['activation_date','개통일'],['status','상태'],['manager','담당'],['memo','메모']],quote=value=>`"${String(value??'').replace(/"/g,'""')}"`,csv='\uFEFF'+[fields.map(x=>quote(x[1])).join(','),...exportRows().map(row=>fields.map(x=>quote(row[x[0]])).join(','))].join('\r\n');download('NRC-고객목록.csv',csv,'text/csv;charset=utf-8')};
  ...
  const selectRepurchase=row=>{repurchaseTarget=row;$('repurchaseSelected').textContent=`${row.name} (개통 ${phone(row.phone)||'-'} · 연락 ${phone(row.contact_phone)||'-'}) 고객`;$('repurchaseAmountForm').hidden=false;$('repurchaseValue').value='';$('repurchaseDate').value=localDate();$('repurchaseNpay').checked=false;$('repurchaseValue').focus()};
  $('repurchasePhone').oninput=()=>{
    ...
    const matches=rows.filter(r=>r.owner_id===me.id&&[r.phone,r.contact_phone].some(value=>String(value||'').replace(/\D/g,'').endsWith(digits)));
    $('repurchaseMatches').innerHTML=matches.length?matches.map(r=>`<button type="button" class="repurchase-match" data-id="${esc(r.id)}">${esc(r.name)} <small>개통 ${esc(phone(r.phone)||'-')} · 연락 ${esc(phone(r.contact_phone)||'-')}</small></button>`).join(''):'<p class="help">일치하는 고객이 없습니다.</p>';
  ...
  $('repurchaseSkip').onclick=()=>{repurchaseTarget=null;$('repurchaseSelected').textContent='특정 고객 지정 없이 등록합니다.';$('repurchaseAmountForm').hidden=false;$('repurchaseValue').value='';$('repurchaseDate').value=localDate();$('repurchaseNpay').checked=false;$('repurchaseValue').focus()};
  $('repurchaseAmountForm').onsubmit=async e=>{e.preventDefault();const amount=Number($('repurchaseValue').value||0);if(amount>0)await recordTransfer('repurchase',amount,repurchaseTarget?{id:repurchaseTarget.id,name:repurchaseTarget.name}:null,$('repurchaseDate').value||localDate(),$('repurchaseNpay').checked);repurchaseDialog.close()};
  await load();
  renderTransferList();
