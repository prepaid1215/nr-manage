...
const vcardPhoneLines=row=>{const seen=new Set();return[[row.phone,'개통번호'],[row.contact_phone,'연락가능번호']].filter(([value])=>{const digits=String(value||'').replace(/\D/g,'');if(!digits||seen.has(digits))return false;seen.add(digits);return true}).map(([value,label],index)=>`item${index+1}.TEL;TYPE=CELL:${phone(value)}\r\nitem${index+1}.X-ABLabel:${label}`).join('\r\n')};
