...
assert.deepEqual(closingPeriodForDate("2026-09-01"), {
  periodId: "2026-09-1",
  year: 2026,
  month: 9,
  round: 1,
...
assert.equal(evaluatePromotion("회원", { firstRoundDtGroupNv: 400000 }).achieved, true);
assert.equal(evaluatePromotion("DT", { firstRoundDtGroupNv: 999999 }).achieved, false);
assert.equal(
  evaluatePromotion("GD", {
    round: 2,
    previousMinorNv: 1200000,
    currentMinorNv: 1800000,
    gdEligible: true,
  }).achieved,
  true,
);
assert.equal(
  evaluatePromotion("RD", { leftQualifiedCount: 2, rightQualifiedCount: 1 }).achieved,
  false,
);
assert.equal(
  evaluatePromotion("ED", { leftQualifiedCount: 2, rightQualifiedCount: 2 }).targetRank,
  "DD",
);
assert.equal(
  evaluatePromotionPath("회원", {
    round: 1,
    firstRoundDtGroupNv: 1000000,
    currentMinorNv: 3000000,
  }).achievedRank,
  "RD",
);
assert.equal(
  evaluatePromotionPath("RD", {
    leftQualifiedCount: 2,
    rightQualifiedCount: 2,
  }).targetRank,
  "ED",
);
...
// ──── 새 규칙 — 선택한 회원만 독립 목표를 갖고 비마감 중간 회원은 delta만 통과 ────
const explicitPayload = {
  rstLst: [
    { userId: "top", userName: "김땡땡", ppId: "", abPos: 0 },
    { userId: "middle", userName: "주땡땡", ppId: "top", abPos: 1 },
    { userId: "other", userName: "박땡땡", ppId: "top", abPos: 2 },
    { userId: "bottom", userName: "이땡땡", ppId: "middle", abPos: 1 },
    { userId: "left", userName: "최땡땡", ppId: "bottom", abPos: 1 },
    { userId: "right", userName: "정땡땡", ppId: "bottom", abPos: 2 },
  ],
  members: [
    { userId: "top", ordPv: 0, maxPv: 100000, minPv: 90000 },
    { userId: "middle", ordPv: 0, maxPv: 50000, minPv: 30000 },
    { userId: "other", ordPv: 0, maxPv: 0, minPv: 90000 },
    { userId: "bottom", ordPv: 10000, maxPv: 30000, minPv: 20000 },
    { userId: "left", ordPv: 0, maxPv: 0, minPv: 30000 },
    { userId: "right", ordPv: 0, maxPv: 0, minPv: 20000 },
  ],
};
const explicitConfigs = {
  top: { closingEnabled: true, majorTargetNv: 120000, minorTargetNv: 100000 },
  middle: { closingEnabled: false, majorTargetNv: 999999, minorTargetNv: 999999 },
  bottom: { closingEnabled: true, majorTargetNv: 40000, minorTargetNv: 40000 },
};
assert.deepEqual(Object.keys(normalizeClosingConfigs(explicitConfigs)).sort(), [
  "bottom",
  "top",
]);
const explicitPlan = planExplicitClosings(
  buildPerformanceModel(explicitPayload),
  "top",
  explicitConfigs,
);
assert.deepEqual(
  explicitPlan.steps.map((step) => step.memberId),
  ["bottom", "top"],
);
assert.equal(
  explicitPlan.steps.some((step) => step.memberId === "middle"),
  false,
);
assert.equal(explicitPlan.topStep.targets.majorTargetNv, 120000);
assert.equal(explicitPlan.topStep.targets.minorTargetNv, 100000);
assert.equal(explicitPlan.currentAchieved, false);
assert.equal(explicitPlan.projectedAchieved, true);
assert.ok(explicitPlan.placements.every((row) => row.salesWon >= 10000));
assert.ok(explicitPlan.placements.every((row) => row.salesWon % 1000 === 0));

assert.throws(
  () =>
    planExplicitClosings(buildPerformanceModel(explicitPayload), "top", {
      top: { closingEnabled: true, majorTargetNv: 120000, minorTargetNv: 0 },
    }),
  /대·소목표를 모두 입력/,
);
